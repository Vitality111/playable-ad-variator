...
function App() {
  ...
  function extractEditableText(html) {
    ...
    $("body *").each((i, el) => {
      ...
      // ⛔ Не змінюємо DOM у службових тегах
      ...
      $el.contents().each((_, node) => {
        if (node.type === "text" && node.data.trim().length > 0) {
          ...
          // Залишаємо всі інші вузли як є
          newChildren.push($.html(node));
        }
      });

      if (newChildren.length > 0) {
        $el.html(newChildren.join(""));
      }
    });
  ...
  const applyTextChanges = () => {
    ...
    editableTextItems.forEach((item) => {
      ...
      if ($el.length === 0) {
        console.warn("❗ Елемент не знайдено:", item.selector);
      } else {
        console.log("✅ Елемент знайдено:", item.selector);
      }
    });
